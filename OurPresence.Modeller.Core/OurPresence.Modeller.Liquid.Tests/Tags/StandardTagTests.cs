...
{%endfor%}";
        ...
        {
            Assert.Equal("variable", Template.Parse("{% assign a = 'variable' %}{{a}}").Render());
        }

        [Fact]
        public void TestAssignAnEmptyString()
        {
            Assert.Equal("", Template.Parse("{% assign a = '' %}{{a}}").Render());
        }

        [Fact]
        public void TestAssignIsGlobal()
        {
            Assert.Equal("variable", Template.Parse("{%for i in (1..2) %}{% assign a = 'variable'%}{% endfor %}{{a}}").Render());
        }
