...
using Range = OurPresence.Liquid.Util.Range;
    ...
    public class Context
    {
        ...
        private object Resolve(string key, bool notifyNotFound = true)
        {
            ...
            if (match.Success)
            {
                ...
                if (decimal.TryParse(match.Groups[1].Value, NumberStyles.Number | NumberStyles.Float, FormatProvider, out var parsedDecimalCurrentCulture))
                    return parsedDecimalCurrentCulture;
                if (decimal.TryParse(match.Groups[1].Value, NumberStyles.Number | NumberStyles.Float, CultureInfo.InvariantCulture, out var parsedDecimalInvariantCulture))
                    return parsedDecimalInvariantCulture;
                if (double.TryParse(match.Groups[1].Value, NumberStyles.Number | NumberStyles.Float, FormatProvider, out var parsedDouble))
                    return parsedDouble;
        ...
        private object Variable(string markup, bool notifyNotFound)
        {
            ...
            {
                if (notifyNotFound)
                    Errors.Add(new VariableNotFoundException(string.Format("Error - Variable '{0}' could not be found", markup)));
                return null;
            }
